...
const getBlogPosts = async limit => {
  const queryWithLimitedNumber = `*[_type == "post"] | order(publishedAt desc, _createdAt desc) [0...${limit}]{
    _createdAt,
    _updatedAt,
    _id,
    title,
  ...
  const queryAll = `*[_type == "post"] | order(publishedAt desc, _createdAt desc){
    _createdAt,
    _updatedAt,
    _id,
    title,
...
const getAllBlogPostsMetas = async () => {
  const queryAll = `*[_type == "post"] | order(publishedAt desc, _createdAt desc){
    publishedAt,
    _createdAt,
    "slug":slug.current}`
...
export { getAllBlogPostsMetas, getBlogPosts }
